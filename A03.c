...
{
  /*
    Clarificações: symbol_t refere-se à estrutura que guarda um bit, o seu parent, e o codename
                   símbolo refere-se ào símbolo inteiro do alfabeto não codificado

    o _c_->data é uma espécie de dicionário, que faz corresponder o índice dado ao symbol_t do símbolo
    (exemplo: data[3] dá-nos o ínicio do código do símbolo 3). Isto devolve apenas o primeiro symbol_t do código.
    Cada symbol_t contém a informação de um bit, e de um parent - que indica o próximo índice do array para obter
    o bit seguinte do código. O primeiro symbol_t de cada símbolo indica também no codename o string correspondente
    ao código completo.


    Exemplo da descoberta de um código:
    Sendo _c_->data assim:
    ...
    Para descobrir o código do símbolo 1, temos que o primeiro bit é 0 (data[1].bit), e o próximo index é 3 (data[1].parent)
    O segundo bit é 1 (data[3].bit), e o próximo bit é 4 (data[3].parent)
    Segamos ao final do código, pois data[4].bit é -1. Temos assim que a codificação do símbolo 1 é 01, tal como indicado em data[1].codename

  */


  // USAR UMA BREADTH-FIRST RECURSION???
  ...
  int parent, bit_idx, next_bit, symbol, idx;

  bit_idx = 1;

  //parent = _c_->data[encoded_idx].parent;
  //bit = _c_->data[encoded_idx].bit;
  
  /*
  for (int idx = 0; idx < _c_->n_symbols; idx++) {
    parent = _c_->data[idx].parent;
    bit = _c_->data[idx].bit;
    next_bit = _c_->data[parent];
